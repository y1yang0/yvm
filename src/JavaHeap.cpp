...
JavaHeap::~JavaHeap() {
	...
	}
	{
		...
		for (auto start = monitorheap.begin(); start != monitorheap.end(); ++start) {
			delete start->second;
		}
	}
}
	...
	if (javaClass.raw.superClass != 0) {
		...
		FOR_EACH(i, superClass->raw.fieldsCount) {
			...
			const char* descriptor =
			    superClass->getString(superClass->raw.fields[i].descriptorIndex);
...
JType* JavaHeap::getObjectFieldByNameImpl(JavaClass* parsedJc, const char* fieldName, const char* fieldDescriptor, JObject* object, size_t offset) {
	std::lock_guard<std::recursive_mutex> lock(objMtx);
	...
	if (parsedJc->raw.superClass != 0) {
		return getObjectFieldByNameImpl(yrt.ma->findJavaClass(parsedJc->getSuperClassName()), fieldName, fieldDescriptor, object, offset + howManyNonStaticFields);
	}
	return nullptr;
}

void JavaHeap::putObjectFieldByNameImpl(JavaClass* parsedJc, const char* fieldName, const char* fieldDescriptor, JObject* object, JType* value, size_t offset) {
	std::lock_guard<std::recursive_mutex> lock(objMtx);
	size_t howManyNonStaticFields = 0;
	FOR_EACH(i, parsedJc->raw.fieldsCount) {
		if (!IS_FIELD_STATIC(parsedJc->raw.fields[i].accessFlags)) {
	...
	if (parsedJc->raw.superClass != 0) {
		putObjectFieldByNameImpl(yrt.ma->findJavaClass(parsedJc->getSuperClassName()), fieldName, fieldDescriptor, object, value, offset + howManyNonStaticFields);
	}
}
