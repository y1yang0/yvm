...
void Inspector::printConstantPool(const JavaClass& jc) {
	...
	for (int i = 1; i <= jc.raw.constPoolCount - 1; i++) {
		...
		if (typeid(*jc.raw.constPoolInfo[i]) == typeid(CONSTANT_Utf8)) {
			d.addCell(
			    (char*) dynamic_cast<CONSTANT_Utf8*>(jc.raw.constPoolInfo[i])->bytes);
		} else if (typeid(*jc.raw.constPoolInfo[i]) == typeid(CONSTANT_String)) {
			d.addCell((char*) jc.getString(
			    dynamic_cast<CONSTANT_String*>(jc.raw.constPoolInfo[i])->stringIndex));
			...
			d.addCell((char*) jc.getString(
			    dynamic_cast<CONSTANT_NameAndType*>(
			        jc.raw.constPoolInfo[dynamic_cast<CONSTANT_Fieldref*>(jc.raw.constPoolInfo[i])
			                                 ->nameAndTypeIndex])
			        ->nameIndex));
		} else if (typeid(*jc.raw.constPoolInfo[i]) == typeid(CONSTANT_Methodref)) {
			d.addCell((char*) jc.getString(dynamic_cast<CONSTANT_NameAndType*>(
			...
			d.addCell((char*) jc.getString(
			    dynamic_cast<CONSTANT_Class*>(jc.raw.constPoolInfo[i])->nameIndex));
		} else if (typeid(*jc.raw.constPoolInfo[i]) == typeid(CONSTANT_NameAndType)) {
			std::string nameAndType;
			nameAndType += (char*) jc.getString(
			    dynamic_cast<CONSTANT_NameAndType*>(jc.raw.constPoolInfo[i])->nameIndex);
			nameAndType += " ! ";
			nameAndType += (char*) jc.getString(
...
void Inspector::printInterfaces(const JavaClass& jc) {
	...
	FOR_EACH(i, jc.raw.interfacesCount) {
		d.addCell("#" + std::to_string(i));
		d.addCell((char*) jc.getString(
		    dynamic_cast<CONSTANT_Class*>(jc.raw.constPoolInfo[jc.raw.interfaces[i]])
		        ->nameIndex));
	}
	d.show();
}
