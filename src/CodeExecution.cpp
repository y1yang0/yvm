	...
	for (decltype(ext.codeLength) op = 0; op < ext.codeLength; op++) {
		...
		case op_ldc2_w: {
			...
			if (typeid(*jc->raw.constPoolInfo[index]) == typeid(CONSTANT_Double)) {
				auto val =
				    dynamic_cast<CONSTANT_Double*>(jc->raw.constPoolInfo[index])->val;
				JDouble* dval = new JDouble;
				dval->val     = val;
				currentFrame->stack.push_back(dval);
			} else if (typeid(*jc->raw.constPoolInfo[index]) == typeid(CONSTANT_Long)) {
				auto val =
				    dynamic_cast<CONSTANT_Long*>(jc->raw.constPoolInfo[index])->val;
				JLong* lval = new JLong;
				lval->val   = val;
				currentFrame->stack.push_back(lval);
			} else {
				throw std::runtime_error("invalid symbolic reference index on "
				                         "constant pool");
			}
		} break;
		case op_iload: {
		...
		case op_aload_3: {
			...
			[[fallthrough]]
#endif
			case op_saload : case op_caload : case op_baload : case op_iaload : {
		...
		case op_dup2_x2: {
			...
			if (IS_COMPUTATIONAL_TYPE_1(value1) && IS_COMPUTATIONAL_TYPE_1(value2) && IS_COMPUTATIONAL_TYPE_1(value3) && IS_COMPUTATIONAL_TYPE_1(value4)) {
				// use structure 1
				currentFrame->stack.push_back(cloneValue(value2));
				currentFrame->stack.push_back(cloneValue(value1));
		...
		case op_swap: {
			...
			if (typeid(*value1) == typeid(JInt) && typeid(*value2) == typeid(JInt)) {
				...
				dynamic_cast<JInt*>(value1)->val =
				    static_cast<int32_t>(dynamic_cast<JFloat*>(value2)->val);
		...
		case op_getfield: {
			...
			auto symbolicRef   = parseFieldSymbolicReference(jc, index);
			JType* field       = cloneValue(yrt.jheap->getObjectFieldByName(std::get<0>(symbolicRef), std::get<1>(symbolicRef), std::get<2>(symbolicRef), objectref));
			currentFrame->stack.push_back(field);
		...
		case op_putfield: {
			...
			JObject* objectref = currentStackPop<JObject>();
			auto symbolicRef   = parseFieldSymbolicReference(jc, index);
			yrt.jheap->putObjectFieldByName(std::get<0>(symbolicRef), std::get<1>(symbolicRef), std::get<2>(symbolicRef), objectref, value);
		...
		case op_invokevirtual: {
			...
			if (strcmp(std::get<1>(symbolicRef), "<init>") == 0) {
				std::runtime_error("invoking method should not be instance "
		...
		case op_anewarray: {
			...
			JArray* arrayref =
			    yrt.jheap->createObjectArray(*std::get<0>(symbolicRef), count->val);
		...
		case op_impdep2: {
			// Reserved opcodde
			std::cerr << "Are you a dot.class hacker? Or you were entered a "
			             "strange region.";
			std::exit(EXIT_FAILURE);
		} break;
		default:
			std::cerr << "The YVM can not recognize this opcode. Bytecode file "
			             "was be corrupted.";
			std::exit(EXIT_FAILURE);
		}
	}
...
void CodeExecution::loadConstantPoolItem2Stack(const JavaClass* jc, u2 index) {
	if (typeid(*jc->raw.constPoolInfo[index]) == typeid(CONSTANT_Integer)) {
		...
		auto val = jc->getString(
		    dynamic_cast<CONSTANT_String*>(jc->raw.constPoolInfo[index])->stringIndex);
		JObject* str = yrt.jheap->createObject(*yrt.ma->loadClassIfAbsent("java"
		                                                                  "/lan"
		                                                                  "g/"
		                                                                  "Stri"
		                                                                  "n"
		...
		throw std::runtime_error("invalid symbolic reference index on constant "
		                         "pool");
	}
}
...
std::tuple<JavaClass*, const char*, const char*> CodeExecution::parseFieldSymbolicReference(const JavaClass* jc, u2 index) const {
	const char* symbolicReferenceFieldName = jc->getString(
	    dynamic_cast<CONSTANT_NameAndType*>(
	        jc->raw.constPoolInfo[dynamic_cast<CONSTANT_Fieldref*>(jc->raw.constPoolInfo[index])
	                                  ->nameAndTypeIndex])
	        ->nameIndex);
	...
	return std::make_tuple(symbolicReferenceClass, symbolicReferenceFieldName, symbolicReferenceFieldDescriptor);
}

std::tuple<JavaClass*, const char*, const char*> CodeExecution::parseInterfaceMethodSymbolicReference(const JavaClass* jc, u2 index) const {
	const char* symbolicReferenceInterfaceMethodName = jc->getString(
	    dynamic_cast<CONSTANT_NameAndType*>(jc->raw.constPoolInfo[dynamic_cast<CONSTANT_InterfaceMethodref*>(
	...
	JavaClass* symbolicReferenceInterfaceMethodClass = yrt.ma->loadClassIfAbsent(jc->getString(
	    dynamic_cast<CONSTANT_Class*>(jc->raw.constPoolInfo[dynamic_cast<CONSTANT_InterfaceMethodref*>(
	                                                            jc->raw.constPoolInfo[index])
	                                                            ->classIndex])
	        ->nameIndex));
	yrt.ma->linkClassIfAbsent(symbolicReferenceInterfaceMethodClass->getClassName());

	return std::make_tuple(symbolicReferenceInterfaceMethodClass, symbolicReferenceInterfaceMethodName, symbolicReferenceInterfaceMethodDescriptor);
}

std::tuple<JavaClass*, const char*, const char*> CodeExecution::parseMethodSymbolicReference(const JavaClass* jc, u2 index) const {
	const char* symbolicReferenceMethodName = jc->getString(
	    dynamic_cast<CONSTANT_NameAndType*>(
	        jc->raw.constPoolInfo[dynamic_cast<CONSTANT_Methodref*>(jc->raw.constPoolInfo[index])
	                                  ->nameAndTypeIndex])
	        ->nameIndex);
	...
	return std::make_tuple(symbolicReferenceMethodClass, symbolicReferenceMethodName, symbolicReferenceMethodDescriptor);
}

std::tuple<JavaClass*> CodeExecution::parseClassSymbolicReference(const JavaClass* jc, u2 index) const {
	const char* ref = jc->getString(
...
JObject* CodeExecution::execNew(const JavaClass* jc, u2 index) {
	...
	if (typeid(*jc->raw.constPoolInfo[index]) != typeid(CONSTANT_Class)) {
		throw std::runtime_error("operand index of new is not a class or "
		                         "interface\n");
	}
	char* className = (char*) jc->getString(
	    dynamic_cast<CONSTANT_Class*>(jc->raw.constPoolInfo[index])->nameIndex);
	...
	FOR_EACH(i, m->attributeCount) {
		if (typeid(*m->attributes[i]) == typeid(ATTR_Code)) {
			...
			ext.exceptionTableLength =
			    dynamic_cast<ATTR_Code*>(m->attributes[i])->exceptionTableLength;
...
bool CodeExecution::checkInstanceof(const JavaClass* jc, u2 index, JType* objectref) {
	std::string TclassName =
	    (char*) dynamic_cast<CONSTANT_Utf8*>(
	        jc->raw.constPoolInfo[dynamic_cast<CONSTANT_Class*>(jc->raw.constPoolInfo[index])
	                                  ->nameIndex])
	        ->bytes;
		...
		if (!IS_CLASS_INTERFACE(dynamic_cast<JObject*>(objectref)->jc->raw.accessFlags)) {
			...
			if (tType == TYPE_CLASS) {
				if (strcmp(yrt.ma
				    ...
				    strcmp(yrt.ma
				               ->findJavaClass(
				                   dynamic_cast<JObject*>(objectref)->jc->getSuperClassName())
				               ->getClassName(),
				           TclassName.c_str()) == 0) {
					return true;
				}
				...
				FOR_EACH(i, interfaceIdxs.size()) {
					auto* interfaceName = dynamic_cast<JObject*>(objectref)->jc->getString(
					    dynamic_cast<CONSTANT_Class*>(
					        dynamic_cast<JObject*>(objectref)->jc->raw.constPoolInfo[interfaceIdxs[i]])
					        ->nameIndex);
			...
			if (tType == TYPE_CLASS) {
				...
				if (strcmp(TclassName.c_str(),
				           dynamic_cast<JObject*>(objectref)->jc->getClassName()) == 0 ||
				    strcmp(TclassName.c_str(),
				           dynamic_cast<JObject*>(objectref)->jc->getSuperClassName()) == 0) {
					return true;
				}
			} else {
				SHOULD_NOT_REACH_HERE
		...
		if (tType == TYPE_CLASS) {
			...
			FOR_EACH(i, interfaceIdxs.size()) {
				if (strcmp(firstComponent->jc->getString(
				               dynamic_cast<CONSTANT_Class*>(
				                   firstComponent->jc->raw.constPoolInfo[interfaceIdxs[i]])
				                   ->nameIndex),
				           TclassName.c_str()) == 0) {
					return true;
				}
	...
	if (jc->raw.interfacesCount > 0) {
		FOR_EACH(eachInterface, jc->raw.interfacesCount) {
			const auto* interfaceName = jc->getString(
			    dynamic_cast<CONSTANT_Class*>(jc->raw.constPoolInfo[jc->raw.interfaces[eachInterface]])
			        ->nameIndex);
...
void CodeExecution::pushMethodArguments(Frame* frame, std::vector<int>& parameter) {
	for (int64_t i = (int64_t) parameter.size() - 1; i >= 0; i--) {
		if (parameter[i] == T_INT || parameter[i] == T_BOOLEAN || parameter[i] == T_CHAR || parameter[i] == T_BYTE || parameter[i] == T_SHORT) {
...
void CodeExecution::invokeInterface(const JavaClass* jc, const char* methodName, const char* methodDescriptor) {
	...
	if (IS_METHOD_NATIVE(invokingMethod.first->accessFlags)) {
		returnValue =
		    cloneValue(execNative(const_cast<JavaClass*>(invokingMethod.second)->getClassName(), methodName, methodDescriptor));
	} else {
