...
public:
	...
	inline JType* getObjectFieldByName(JavaClass* parsedJc, const char* fieldName, const char* fieldDescriptor, JObject* object) {
		return getObjectFieldByNameImpl(parsedJc, fieldName, fieldDescriptor, object, 0);
	}
	inline void putObjectFieldByName(JavaClass* parsedJc, const char* fieldName, const char* fieldDescriptor, JObject* object, JType* value) {
...
private:
	 ...
	 */
	JType* getObjectFieldByNameImpl(JavaClass* parsedJc, const char* fieldName, const char* fieldDescriptor, JObject* object, size_t offset = 0);
	void putObjectFieldByNameImpl(JavaClass* parsedJc, const char* fieldName, const char* fieldDescriptor, JObject* object, JType* value, size_t offset = 0);

private:
	map<size_t, vector<JType*>, less<>, HeapAllocator<pair<const size_t, vector<JType*>>>> objheap;
	recursive_mutex objMtx;

	map<size_t, pair<size_t, JType**>, less<>, HeapAllocator<pair<const size_t, pair<size_t, JType**>>>> arrheap;
	recursive_mutex arrMtx;

	map<size_t, ObjectMonitor*, less<>, HeapAllocator<pair<const size_t, ObjectMonitor*>>> monitorheap;
