...
std::vector<u2> JavaClass::getInterfacesIndex() const {
	...
	FOR_EACH(i, raw.interfacesCount) {
		v.push_back(
		    dynamic_cast<CONSTANT_Class*>(raw.constPoolInfo[raw.interfaces[i]])->nameIndex);
	}
	return v;
}
	...
	for (int i = 1; i <= cpCount - 1; i++) {
		...
		case TAG_InterfaceMethodref: {
			...
			dynamic_cast<CONSTANT_InterfaceMethodref*>(slot)->classIndex =
			    reader.readget2();
			dynamic_cast<CONSTANT_InterfaceMethodref*>(slot)->nameAndTypeIndex =
			    reader.readget2();
		...
		case TAG_Long: {
			...
			dynamic_cast<CONSTANT_Long*>(slot)->val =
			    (((int64_t) dynamic_cast<CONSTANT_Long*>(slot)->highBytes) << 32) +
		...
		case TAG_InvokeDynamic: {
			...
			dynamic_cast<CONSTANT_InvokeDynamic*>(slot)->bootstrapMethodAttrIndex =
			    reader.readget2();
...
bool JavaClass::parseAttribute(AttributeInfo**(&attrs), u2 attributeCount) {
	...
	for (decltype(attributeCount) i = 0; i < attributeCount; i++) {
		...
		char* attrName = (char*) dynamic_cast<CONSTANT_Utf8*>(raw.constPoolInfo[attrStrIndex])
		                     ->bytes;
			...
			FOR_EACH(k, attr->numberOfEntries) {
				...
				if (IS_STACKFRAME_same_frame(frameType)) {
					...
					FOR_EACH(p, frameType - 251) {
						frame->stack[p] = determineVerificationType(reader.readget1());
					}
					attr->entries[k] = frame;
					...
					FOR_EACH(p, frame->numberOfLocals) {
						frame->locals[p] = determineVerificationType(reader.readget1());
					}
		...
		IS_ATTR_LocalVariableTypeTable(attrName) {
			...
			FOR_EACH(k, attr->localVariableTypeTableLength) {
				...
				attr->localVariableTypeTable[k].signatureIndex = reader.readget2();
		...
		IS_ATTR_RuntimeVisibleParameterAnnotations(attrName) {
			...
			FOR_EACH(k, attr->numParameters) {
				...
				attr->parameterAnnotations[k].annotations =
				    new Annotation[attr->parameterAnnotations[k].numAnnotations];
		...
		IS_ATTR_RuntimeVisibleTypeAnnotations(attrName) {
			...
			FOR_EACH(k, attr->numAnnotations) {
				...
				attr->annotations[k].targetPath.path =
				    new TypeAnnotation::TypePath::_path[attr->annotations[k]
				...
				attr->annotations[k].elementValuePairs =
				    new TypeAnnotation::_elementValuePairs[attr->annotations[k].numElementValuePairs];
		...
		IS_ATTR_BootstrapMethods(attrName) {
			...
			FOR_EACH(k, attr->numBootstrapMethods) {
				...
				attr->bootstrapMethod[k].numBootstrapArgument = reader.readget2();
				...
				FOR_EACH(p, attr->bootstrapMethod[k].numBootstrapArgument) {
					attr->bootstrapMethod[k].bootstrapArguments[p] = reader.readget2();
				}
			}
			attrs[i] = attr;
			continue;
