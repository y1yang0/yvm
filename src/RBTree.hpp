...
class RBTree {
    using _ResolvedKeyType = typename std::conditional<
        std::is_class<_KeyType>::value,
        typename std::add_lvalue_reference<_KeyType>::type, _KeyType>::type;

    using _ResolvedValueType = typename std::conditional<
        std::is_class<_ValueType>::value,
        typename std::add_lvalue_reference<_ValueType>::type, _ValueType>::type;
...
private:
    bool isRed(Node* node) const { return node->black == false; }

    bool isBlack(Node* node) const { return node->black == true; }

    bool exist(Node* node) const { return node != nullptr; }
...
template <typename _KeyType, typename _ValueType>
void RBTree<_KeyType, _ValueType>::add(_ResolvedKeyType key,
                                       _ResolvedValueType value) {
    Node* traverse = root;
...
template <typename _KeyType, typename _ValueType>
typename RBTree<_KeyType, _ValueType>::Node*
RBTree<_KeyType, _ValueType>::minNode(Node* start) {
    if (root == nullptr) {
        return nullptr;
    }
    ...
    while (current && current != root && current->black == true) {
        if (current == current->parent->left) {
            ...
            if (exist(brother)) {
                if (isRed(brother)) {
                ...
                } else if ((isBlack(brother) && !exist(brother->left) &&
                            !exist(brother->right)) ||
                           (isBlack(brother) && exist(brother->left) &&
                            isBlack(brother->left) && !exist(brother->right)) ||
                           (isBlack(brother) && exist(brother->right) &&
                            isBlack(brother->right) && !exist(brother->left)) ||
                           (isBlack(brother) && exist(brother->right) &&
                            isBlack(brother->right) && exist(brother->left) &&
                    ...
                    current = current->parent;
                            ...
                            isRed(brother->left) && exist(brother->right) &&
                    ...
                    rotateRight(brother);
                    brother = current->parent->right;
                } else if (exist(brother->right) && isBlack(brother) &&
                           isRed(brother->right)) {
            ...
            if (exist(brother)) {
                if (isRed(brother)) {
                    ...
                    brother->black = false;
                    current = current->parent;
                } else if ((isBlack(brother) && exist(brother->right) &&
                            isRed(brother->right) && exist(brother->left) &&
                            isBlack(brother->left)) ||
                           (isBlack(brother) && exist(brother->right) &&
                            isRed(brother->right) && !exist(brother->left))) {
                    brother->right->black = true;
                    ...
                    rotateLeft(brother);
                    brother = current->parent->left;
                } else if (exist(brother->left) && isBlack(brother) &&
                           isRed(brother->left)) {
                    brother->black = current->parent->black;
                    current->parent->black = true;
                    brother->left->black = true;
