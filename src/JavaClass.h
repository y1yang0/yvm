...
public:
	...
	const char* getClassName() const {
		return getString(
		    dynamic_cast<CONSTANT_Class*>(raw.constPoolInfo[raw.thisClass])->nameIndex);
	}

	const char* getSuperClassName() const {
		return raw.superClass == 0 ? nullptr :
		                             getString(dynamic_cast<CONSTANT_Class*>(raw.constPoolInfo[raw.superClass])
		                                           ->nameIndex);
	}
