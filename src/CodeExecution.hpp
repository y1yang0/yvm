...
private:
	std::pair<MethodInfo*, const JavaClass*> findMethod(const JavaClass* jc, const char* methodName, const char* methodDescriptor);
	...
	std::tuple<JavaClass*, const char*, const char*> parseInterfaceMethodSymbolicReference(const JavaClass* jc, u2 index) const;
...
private:
	template <typename LoadType> void load2Stack(u1 localIndex);
	template <typename LoadType> void loadArrayItem2Stack();
	template <typename StoreType> void store2Local(u1 index);
	template <typename StoreType> void storeArrayItem();
	template <typename ReturnType> ReturnType* flowReturn() const;
	template <typename ReturnType> ReturnType* currentStackPop();
	...
	template <typename Type1, typename Type2> void typeCast() const;
...
template <typename LoadType> void CodeExecution::load2Stack(u1 localIndex) {
...
template <> inline void CodeExecution::load2Stack<JRef>(u1 localIndex) {
	JType* pushingV{};
	if (typeid(*currentFrame->locals[localIndex]) == typeid(JObject)) {
		pushingV = new JObject;
		dynamic_cast<JObject*>(pushingV)->jc =
		    dynamic_cast<JObject*>(currentFrame->locals[localIndex])->jc;
		dynamic_cast<JObject*>(pushingV)->offset =
		    dynamic_cast<JObject*>(currentFrame->locals[localIndex])->offset;
	} else if (typeid(*currentFrame->locals[localIndex]) == typeid(JArray)) {
		pushingV = new JArray;
		dynamic_cast<JArray*>(pushingV)->length =
		    dynamic_cast<JArray*>(currentFrame->locals[localIndex])->length;
		dynamic_cast<JArray*>(pushingV)->offset =
		    dynamic_cast<JArray*>(currentFrame->locals[localIndex])->offset;
	} else {
...
template <typename LoadType> void CodeExecution::loadArrayItem2Stack() {
	...
	ival->val =
	    dynamic_cast<LoadType*>(yrt.jheap->getArrayItem(*arrayref, index->val))->val;
...
template <> inline void CodeExecution::loadArrayItem2Stack<JRef>() {
	...
	if (typeid(*ptrArrItem) == typeid(JArray)) {
		...
		dynamic_cast<JArray*>(objectref)->length =
		    dynamic_cast<JArray*>(ptrArrItem)->length;
		dynamic_cast<JArray*>(objectref)->offset =
		    dynamic_cast<JArray*>(ptrArrItem)->offset;
	} else if (typeid(*ptrArrItem) == typeid(JObject)) {
		objectref = new JObject;
		dynamic_cast<JObject*>(objectref)->jc =
		    dynamic_cast<JObject*>(ptrArrItem)->jc;
		dynamic_cast<JObject*>(objectref)->offset =
		    dynamic_cast<JObject*>(ptrArrItem)->offset;
	}
...
template <typename StoreType> void CodeExecution::store2Local(u1 index) {
...
template <> inline void CodeExecution::store2Local<JRef>(u1 index) {
	JType* value = currentFrame->stack.back();
	currentFrame->stack.pop_back();
	currentFrame->locals[index] = value;
}

template <typename StoreType> void CodeExecution::storeArrayItem() {
...
template <> inline void CodeExecution::storeArrayItem<JRef>() {
	JType* value = currentFrame->stack.back();
	currentFrame->stack.pop_back();
	JInt* index = (JInt*) currentFrame->stack.back();
	currentFrame->stack.pop_back();
...
template <typename ReturnType> ReturnType* CodeExecution::flowReturn() const {
	auto* value = dynamic_cast<ReturnType*>(currentFrame->stack.back());
	currentFrame->stack.pop_back();
	return value;
}

template <typename ReturnType> ReturnType* CodeExecution::currentStackPop() {
	auto* value = dynamic_cast<ReturnType*>(currentFrame->stack.back());
	currentFrame->stack.pop_back();
	return value;
}
...
template <typename Type1, typename Type2> void CodeExecution::typeCast() const {
